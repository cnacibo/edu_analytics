...
class HSEProgramScraper(BaseScraper):
    """Скрепер для курсов программ ВШЭ (ПИ и ПМИ)"""

    def __init__(self):
        super().__init__(base_url="https://www.hse.ru", name="HSE_Program_Scraper")
        self.program_urls = {
    ...
    def _get_total_pages(self, soup):
        ...
        if active:
            try:
                ...
                logger.info("Ошибка извлечения количества страниц")
        return max_page
    ...
    def _extract_language(self, soup):
        """Извлечение языка, на котором преподается дисциплина"""
    ...
    def _extract_content(self, soup):
        """Извлечение данных 'Содердания учебной дисциплины'"""
    ...
    def _extract_results(self, soup):
        """Извлечение пункта 'Планиуремые результаты обучения'"""

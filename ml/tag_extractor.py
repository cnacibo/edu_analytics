...
Далее класс выполняет следующее:
    ...
    - Фильтрует подходящие слова по длине, стоп-словам, частям речи (через библиотеку pymorphy2) и исключение глаголов
    - Добавление известных терминов из внутреннего словаря (garbage_phrases и self.known_terms)
...
class TagExtractor:
    def __init__(self,
        ...
        self.garbage_phrases = {
            'в конце', 'должен уметь', 'быть в состоянии', 'объяснить',
            'рассказать', 'применять', 'использовать', 'сформулировать',
            'описать', 'обосновать', 'подготовиться', 'понимать',
            'основной', 'компетенция', 'результат', 'обучение',
            'лекция', 'практика', 'семинар', 'лабораторный'
        }

        self.known_terms = {
            'agile', 'safe', 'scrum', 'kanban', 'devops', 'wsjf',
            'art', 'less', 'xp', 'saFe', 'SAFe', 'Agile',
            'бережливый', 'гибкий', 'портфель', 'эпик', 'lean',
            'поток ценности', 'непрерывная поставка', 'pi planning',
            'agile release trains', 'кросс функциональный',
            'бережливое управление', 'цифровые технологии'
        }

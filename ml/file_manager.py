...
def process_csv_file(input_file: str, output_file: str = None):
    ...
    # print(f"Найдено строк: {len(df)}") -> log
    ...
    if output_file is None:
        ...
        # output_file = input_path.parent / f"tags{input_path.suffix}"
        script_dir = Path(__file__).parent
        output_file = script_dir / "tags.csv"

    if title_col is None:
        title_col = content_col
    df_to_save = df[[title_col, 'tags_str']].copy()
    df_to_save.to_csv(output_file, index=False, encoding='utf-8-sig')
    # print(f"\nРезультат сохранён в: {output_file}") -> log
    return df